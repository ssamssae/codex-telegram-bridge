...
class HostNoticeTest(unittest.TestCase):
    """세션 바인딩은 붙기의 전제 조건이 아니다.

    바인딩은 세션 id 로 resume 하는 쪽에나 필요하다. attach 는 raw 를 구독하고 key op 로
    입력할 뿐이라 바인딩 없이도 동작한다 — 이걸 차단 조건으로 쓰면 호스트 재기동 직후처럼
    아직 세션이 없는 창에 붙지 못한다(2026-07-22 실사고).
    """

    def test_unbound_session_is_a_notice_not_a_block(self) -> None:
        notice = attach.host_notice({"ok": True, "session_bound": False})
        self.assertTrue(notice)
        self.assertIn("화면·입력은 그대로", notice)

    def test_bound_session_says_nothing(self) -> None:
        self.assertEqual(attach.host_notice({"ok": True, "session_bound": True}), "")


class ProbeHostRestartTest(unittest.TestCase):
    """호스트 재기동 창에서 죽지 않는가.

    재기동하면 파이프 이름과 generation 이 바뀐다. 쥐고 있던 옛 이름으로 계속 두드리면
    영영 닿지 못하고, 예외가 그대로 올라가면 사용자는 트레이스백을 본다(2026-07-22 실사고).
    """

    def setUp(self) -> None:
        self.original = attach.pipe_request
        self.tmp = tempfile.TemporaryDirectory()
        self.state = Path(self.tmp.name) / "repl-host.json"

    def tearDown(self) -> None:
        attach.pipe_request = self.original
        self.tmp.cleanup()

    def _write_descriptor(self, generation: str) -> None:
        self.state.write_text(
            json.dumps(
                {
                    "schema": 1,
                    "pipe_name": rf"\\.\pipe\codex-repl-host-{generation}",
                    "capability": "cap",
    ...
    def test_reloads_descriptor_and_retries_once(self) -> None:
        stale = {"pipe_name": r"\\.\pipe\old", "capability": "cap", "generation": "old"}
        self._write_descriptor("new")
        seen: list[str] = []

        def fake_request(pipe_name, request, timeout_ms=1500):  # noqa: ANN001
            seen.append(pipe_name)
            if pipe_name.endswith("old"):
                raise TimeoutError("ConPTY host IPC is unavailable")
            return {"ok": True, "session_bound": False, "request_id": request["request_id"]}

        attach.pipe_request = fake_request
        descriptor, probe = attach.probe_host(stale, self.state)

        self.assertTrue(probe["ok"])
        self.assertEqual(descriptor["generation"], "new")
        self.assertEqual(len(seen), 2)  # 옛 이름 1회 → 새 이름 1회, 무한재시도 아님

    def test_missing_descriptor_surfaces_as_timeout_not_traceback(self) -> None:
        def always_timeout(pipe_name, request, timeout_ms=1500):  # noqa: ANN001
            raise TimeoutError("ConPTY host IPC is unavailable")

        attach.pipe_request = always_timeout
        with self.assertRaises(TimeoutError):
            attach.probe_host({"pipe_name": r"\\.\pipe\gone"}, self.state)
